...
export default function AnalyzePage() {
  ...
  const handleConfirmCandidateModels = () => {
  ...
  const handleRunAnalysis = async () => {
    ...
    const payload = {
      ...
      items: visibleSelectedModels.map((model, index) => ({
        order: index,
...
현재까지 수정된 코드입니다. 다음을 수정하려고 해요:
Detailed Comparison Results 그려지는 시점
현재는 사이드바의 Analyze Type 콤보박스에서 type 설정 후 "분석 실행" 버튼을 눌러야 그려짐. 이걸 Analyze Page에서 모델 추가하는 시점(n개 추가 버튼 누르는 시점)에 바로 나도록 변경. Analysis Type에 "기본 분석" 추가하여 이걸로 디폴트로 설정됨


사이드바의 "분석 실행"을 누르는 경우는 사용자가 "기본 분석"외 다른 타입의 Analysis Type을 직접 설정해서 분석을 수행하는 경우로 할 거임​‌

그리고 Detailed Comparison Results 박스랑 summary표 자체에도 스크롤이 생기는데 둘다 내려도 표가 짤려보이는 문제가 있습니다. Detailed Comparison Results 박스에만 스크롤을 남겨주시되 표가 짤려보이는 문제를 없애주세요.

어떤 부분을 어떻게 수정하면 되는지 알려주시면 됩니다.
